...
The L2 Status Factor Model [@bardel_role_2007; @cabrelli_amaro_l2_2012] predicts that the second learned language will block access to the L1, and that the L3 will be influenced solely by the L2. 
On the other hand, the Typological Primacy Model (TPM; @rothman_typological_2010, @rothman_l3_2011, @baauw_cognitive_2013, and @rothman_linguistic_2015) posits that either the L1 or the L2, but not both, will influence the L3, and that the choice of which of these two languages influences the L3 is determined based on perceived typological similarity on the basis of L3 input. 
Other models suggest that whole language influence does not occur, and that gradient effects of both languages is possible such as the Scalpel Model [@slabakova_scalpel_2017] and the Linguistic Proximity Model (LPM; @westergaard_crosslinguistic_2017). 
...
To test for practical equivalence, participant mean relative VOT per language were utilized to carry out paired tests of equivalence (TOST; [@lakens_equivalence_2017]). 
...
Nested model comparisons revealed a main effect for both *language* (X(`r nmc_f$Df[2]`) = `r nmc_f$Chisq[2] %>% round(digits = 2)`; p < .001) and *consonant* (
X(`r nmc_f$Df[3]`) = `r nmc_f$Chisq[3] %>% round(digits = 2)`; p < .001). 
...
The L2 subset group was determined on the basis of two sample t-tests in which individual participants’ French and English relative VOT productions were determined to be inconclusive (p > .05). 
