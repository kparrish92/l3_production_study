...
The input is parsed for cues in a hierarchical manner in the TPM, in which the lexicon is theorized to be the primary cue attended to by the L3 learner, followed by phonological cues, then by morphosyntax  and finally by syntax [@rothman_linguistic_2015].
...
For example, in his seminal paper, @rothman_l3_2011 examined two L3 groups using a one-way ANOVA. 
...
A total of `r total_participants + 30` participants were recruited for the present study.
Two groups of participants were recruited, consisting of an experimental group and a control group. After exclusion of some participants, the experimental group consisted of `r total_participants_q` Mexican Spanish-English bilinguals and the control group consisted of **30** Spanish monolinguals. 
...
Tables 2, 3, 4 and list the relative VOT per consonant in each language and table 5 contains the pooled relative VOT across place of articulation per language for the full data set. 
Table 6 lists the mean and standard devation values of relative VOT productions by the monolingual group in Spanish and French. 
...
Figure 2 plots relative VOT as a function of language and group. 
The wide variation in the French production in the bilingual group can be seen in the plot.
This wide variation is arguably due to English influence of L3 French productions. 

\@ref(fig:figure_1)

```{r, 'figure_1', echo=FALSE, fig.cap="(ref:figure_1)"}
...
TOSTs revealed that the L1 Spanish **t(239.88) = -2.458, p = 0.00733 and L2 English (t(309.9) = -2.832, p = 0.00246)** productions were produced practically equivalent between the subsets. 
...
However, the parameter estimates of the model revealed a smaller effect for language for French  `r l2_fr` relative to English than the full dataset model (`r full_fr`), suggesting that the L2 subset had greater influence of English in their L3 French.
The effect for Spanish was similar `r l2_sp` (full model `r full_fr`). The full L2 subset model can be seen in table 9. 
...
Additionally, the parameter estimates of the model revealed a larger effect for language for French  `r l1_fr` relative to English than the full dataset model (`r full_fr`). The effect for Spanish was similar `r l1_sp` (full model `r full_sp`). 
